...
# Author: GrixisTV
# Usage: .\run-developer-mode-fixed.ps1

# Use the full path to RuneLite.exe
...
# Navigate to the RuneLite directory first
Set-Location "$env:LOCALAPPDATA\RuneLite"

# Run RuneLite in developer mode
Write-Host "Starting RuneLite in developer mode from $PWD..." -ForegroundColor Cyan
try {
    # Start RuneLite directly from its directory
    Start-Process -FilePath ".\RuneLite.exe" -ArgumentList "--developer-mode", "--debug" -NoNewWindow
